(() => {
  ...
  async function guard() {
    if (!window.supabaseClient || !window.hubAuth) {
      window.location.replace("login/?reason=configuration");
      return;
    }
    try {
      const { session, profile } = await window.hubAuth.getCurrentUserAndProfile();
      if (!session) return window.location.replace("login/?reason=session");
      if (!profile) {
        await window.supabaseClient.auth.signOut();
        return window.location.replace("login/?reason=profile");
      }
      const allowed = (document.body.dataset.allowedRoles || "student,teacher,admin").split(",").map(role => role.trim());
      if (!allowed.includes(profile.role)) {
        const destination = window.hubAuth.roleDestination(profile.role) || "login/";
        return window.location.replace(destination);
      }
      window.hubCurrentUser = { session, user: session.user, profile };
      if (profile.role === "student") {
        let dashboardLink=document.querySelector('a.btn[href="student-dashboard/"]:not(.brand), .dashboard-nav a[href="student-dashboard/"]');
        if(dashboardLink){dashboardLink.textContent="My Dashboard";dashboardLink.classList.add("student-dashboard-menu-link");}
          ...
          if(!shell){const header=document.createElement("header");header.className="dashboard-header student-quick-header";header.innerHTML='<div class="container nav-shell"><a class="brand" href="student-dashboard/"><img src="assets/icons/logo.png" alt=""><span class="brand-copy"><strong>Interactive</strong><span>Literacy Hub</span></span></a></div>';document.body.insertBefore(header,document.body.firstChild);shell=header.querySelector(".nav-shell");}
          dashboardLink=document.createElement("a");dashboardLink.href="student-dashboard/";dashboardLink.className="btn btn-secondary student-dashboard-menu-link";dashboardLink.textContent="My Dashboard";shell.append(dashboardLink);
        }
      }
      ...
      window.location.replace(`login/?reason=${navigator.onLine ? "session" : "network"}`);
    }
  }

  window.supabaseClient?.auth.onAuthStateChange((event) => {
    if (event === "SIGNED_OUT") window.location.replace("login/?reason=session");
  });
  guard();
})();
