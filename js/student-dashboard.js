(() => {
  "use strict";
  document.querySelector('.dashboard-nav a[href="reading/"]')?.remove();
  ...
  const cleanActivityUrl = value => String(value || "").replace(/([a-z0-9-]+)\.html(?=([?#]|$))/gi, "$1/");
  const activityUrl = (activity) => cleanActivityUrl(activity.page_url) || ({ vocabulary: "vocabulary/", reading: "reading/", collaboration: "collaborate/", culture: "culture/", technology: "technology/", family: "family/" }[activity.category] || "./");
  const practiceLink=document.querySelector('.dashboard-nav a[href="vocabulary/"]');
  if(practiceLink){practiceLink.href="reading-library/";practiceLink.textContent="Choose Your Reading";}
  const avatarFiles=Array.from({length:6},(_,index)=>`avatar${index+1}.png`);
  const eagleProgressImages=["progrees1.webp","progress2.webp","progress3.webp","progress4.webp","progress5.webp","progress6.webp","progress7.webp","progress8.webp","progress9.webp","progress10.webp"];
  ...
  async function loadDashboard(event) {
    ...
    try {
      ...
      status.textContent = currentJourney ? "Your dashboard is up to date." : "Choose a reading from the library to begin your first journey.";
