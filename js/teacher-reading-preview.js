(() => {
  ...
  function stageBody(stage) {
    ...
    if (stage.type === "text") return `<span class="badge">Read the Text</span><h2>${safe(reading.title)}</h2><figure class="reading-text-cover"><img src="${safe(reading.image)}" alt="Cover for ${safe(reading.title)}"></figure><div class="reading-text">${String(content.text || "").split(/\r?\n/).map(line => line.trim()).filter(Boolean).map(line => `<p>${safe(line)}</p>`).join("")}</div>`;
